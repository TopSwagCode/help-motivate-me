...
public static class OnboardingPrompts
{
    ...
    public static string GetPromptForStep(string step)
    {
    ...
    public static string BuildSystemPrompt(string step, Dictionary<string, object>? context)
    {
        ...
        if (context.TryGetValue("currentYear", out var year)) contextLines.Add($"- Current year: {year}");

        if (context.TryGetValue("dayOfWeek", out var dayOfWeek)) contextLines.Add($"- Day of week: {dayOfWeek}");
        ...
        if (context.TryGetValue("endOfYearDate", out var endOfYear)) contextLines.Add($"- End of year: {endOfYear}");

        if (context.TryGetValue("timeZone", out var timeZone)) contextLines.Add($"- User's timezone: {timeZone}");
        ...
        if (context.TryGetValue("userIdentities", out var identitiesObj) && identitiesObj is JsonElement jsonElement)
        {
            ...
            if (jsonElement.ValueKind == JsonValueKind.Array)
                foreach (var item in jsonElement.EnumerateArray())
                {
                    ...
                    if (!string.IsNullOrEmpty(id) && !string.IsNullOrEmpty(name)) identities.Add((id, name));
                }
            ...
            if (identities.Count > 0)
            {
                ...
                foreach (var (id, name) in identities) contextLines.Add($"- \"{name}\" (id: {id})");
                contextLines.Add("");
                contextLines.Add(
                    "IMPORTANT: When creating habit stacks or goals, you can link them to an identity by including 'identityName' in the data.");
                contextLines.Add("This helps the user connect their habits and goals to who they want to become.");
                contextLines.Add(
                    "Example: If user mentions routines related to fitness, link to a fitness-related identity like \"Athlete\".");
            }
        }
