...
public class DailyCommitmentNotificationService : IDailyCommitmentNotificationService
{
    ...
    public async Task<int> ProcessEligibleUsersAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation(
            "Completed daily commitment notification processing. Sent {NotificationCount} notifications",
            notificationsSent);
    ...
    /// </summary>
    private async Task<List<NotificationCandidate>> GetCandidateUsersAsync(DateTime utcNow,
        ...
        var candidates = await _context.NotificationPreferences
            ...
            .Where(np => !_context.Set<DailyIdentityCommitment>()
                .Any(dc => dc.UserId == np.UserId &&
                           ...
                           (dc.Status == DailyCommitmentStatus.Completed ||
                            dc.Status == DailyCommitmentStatus.Dismissed)))
    ...
    /// </summary>
    private async Task<bool> ProcessUserAsync(NotificationCandidate candidate, DateTime utcNow,
        CancellationToken cancellationToken)
    {
        ...
        if (!IsDaySelected(localDateTime.DayOfWeek, candidate.SelectedDays)) return false;

        // Resolve which time slot we're currently in (if any)
        var currentSlot = ResolveCurrentTimeSlot(localTime, candidate.PreferredTimeSlot, candidate.CustomTimeStart,
            candidate.CustomTimeEnd);
    ...
    /// </summary>
    private static TimeSlot? ResolveCurrentTimeSlot(TimeOnly localTime, TimeSlot preferredSlot, TimeOnly? customStart,
        TimeOnly? customEnd)
    {
