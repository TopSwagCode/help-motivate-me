...
<div class="relative">
	...
	<div class="flex flex-wrap gap-2">
		{#each quickEmojis() as emoji}
			<button
				type="button"
				class="w-10 h-10 text-xl rounded-lg border-2 transition-colors {value === emoji
		...
		<button
			type="button"
			class="w-10 h-10 text-xl rounded-lg border-2 border-gray-200 hover:border-gray-300 transition-colors flex items-center justify-center font-bold text-gray-500 hover:text-gray-700"
...
{#if showFullPicker}
	...
	<div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onclick={closeFullPicker}>
		<!-- svelte-ignore a11y_click_events_have_key_events -->
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col" onclick={(e) => e.stopPropagation()}>
			<!-- Header -->
			<div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
				<h3 class="text-lg font-semibold text-gray-900">Choose an Emoji</h3>
				<button
					type="button"
						...
						<div class="mb-6">
							...
							<div class="grid grid-cols-8 sm:grid-cols-10 gap-2">
								{#each emojis as emoji}
									<button
										type="button"
										class="w-10 h-10 text-2xl rounded-lg border-2 transition-colors {value === emoji
											? 'border-primary-500 bg-primary-50'
											: 'border-gray-200 hover:border-gray-300'}"
										onclick={() => selectEmoji(emoji)}
									>
