					...
					{#each todayData.identityProgress as progress (progress.id)}
						...
						>
							...
							<div class="flex items-center justify-between mb-1.5">
								...
								{:else}
									<svg class="w-6 h-6 text-primary-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
							...
							<div class="flex items-center gap-1.5 mb-1.5">
								<p class="text-xs font-medium text-cocoa-700 truncate" title={progress.name}>{progress.name}</p>
								<span class="text-[10px] text-cocoa-500 flex-shrink-0">{$t('today.status.' + progress.status.toLowerCase())}</span>
							</div>
							<!-- Mini progress bar -->
