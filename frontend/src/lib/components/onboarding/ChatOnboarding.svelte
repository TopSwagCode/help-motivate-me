		...
		if (data.action === 'create') {
			...
			if (type === 'identity') {
				...
				if (Array.isArray(dataObj.items) && dataObj.items.length > 0) {
					...
					if (!hasName) console.warn('Identity validation failed: first item missing name');
					return hasName;
				}
				// Legacy single item format
				...
				if (Array.isArray(dataObj.stacks) && dataObj.stacks.length > 0) {
					...
					if (!hasName || !hasHabits) console.warn('HabitStack validation failed (stacks format):', { hasName, hasHabits });
				...
				if (!hasName || !hasHabits) console.warn('HabitStack validation failed:', { hasName, hasHabits });
				return hasName && hasHabits;
			} else if (type === 'goal') {
				// Check for items array format first
				if (Array.isArray(dataObj.items) && dataObj.items.length > 0) {
					...
					if (!hasTitle) console.warn('Goal validation failed: first item missing title');
					return hasTitle;
				}
				// Legacy single item format
