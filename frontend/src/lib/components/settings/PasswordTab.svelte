<script lang="ts">
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			success = get(t)('settings.password.success');
			currentPassword = '';
			newPassword = '';
			confirmPassword = '';
		} catch (e) {
			error = e instanceof Error ? e.message : get(t)('settings.password.error');
		} finally {
			loading = false;
		}
	}
...
<div>
	<h2 class="text-lg font-semibold text-cocoa-800 mb-4">{$t('settings.password.title')}</h2>
	...
	<form onsubmit={handleSubmit} class="space-y-4 max-w-md">
		...
		<div>
			<label for="currentPassword" class="label">{$t('settings.password.current')}</label>
			<input
				id="currentPassword"
				type="password"
		...
		<div>
			<label for="newPassword" class="label">{$t('settings.password.new')}</label>
			<input
				id="newPassword"
				type="password"
				...
				class="input"
			/>
			<p class="mt-1 text-xs text-cocoa-500">{$t('settings.password.requirements')}</p>
		</div>

		<div>
			<label for="confirmPassword" class="label">{$t('settings.password.confirm')}</label>
			<input
				id="confirmPassword"
				type="password"
			...
			{#if confirmPassword && !passwordsMatch}
				<p class="mt-1 text-xs text-red-600">{$t('settings.password.mismatch')}</p>
			{/if}
		</div>
