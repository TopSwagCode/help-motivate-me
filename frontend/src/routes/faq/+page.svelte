<script lang="ts">
	...
	const faqSections = [
		...
		{
			id: 'identity-proofs',
			titleKey: 'faq.questions.identityProofs.title',
			contentKey: 'faq.questions.identityProofs.content'
		},
		{
			id: 'habit-stacks',
		...
		<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
			<div class="flex items-center justify-between">
				...
				{#if isLoggedIn}
					...
					>
						&larr; {$t('faq.goBack', { default: 'Go back to app' })}
					</button>
				{:else}
					<a
						href="/auth/login"
	...
	<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
		...
		{#if isLoggedIn}
			<div class="mt-12 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-8 text-center">
				<h2 class="text-2xl font-bold text-white mb-4">{$t('faq.cta.backTitle', { default: 'Ready to continue?' })}</h2>
				<p class="text-indigo-100 mb-6 max-w-2xl mx-auto">
					{$t('faq.cta.backDescription', { default: 'Head back to the app and keep building your habits.' })}
				</p>
				<button
					on:click={() => history.back()}
