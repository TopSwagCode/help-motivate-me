...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
	<div class="max-w-md w-full">
		<div class="text-center mb-8">
			...
			<h1 class="mt-6 text-3xl font-bold text-cocoa-800">{$t('auth.login.heading')}</h1>
			<p class="mt-2 text-cocoa-600">
				{$t('auth.login.noAccount')} <a href="/auth/register" class="text-primary-600 hover:text-primary-500 font-medium">{$t('auth.login.noAccountLink')}</a>
			</p>
		</div>
		...
		<div class="card p-8">
			...
			{#if loading && $page.url.searchParams.get('token')}
				<div class="text-center py-8">
					...
					<p class="text-cocoa-600">{$t('auth.login.signingIn')}</p>
				</div>
			{:else if emailLinkSent}
				<div class="text-center py-8">
					...
					<h2 class="text-xl font-semibold text-cocoa-800 mb-2">{$t('auth.login.checkEmail')}</h2>
					<p class="text-cocoa-600 mb-4">
						{$t('auth.login.sentLink')} <strong>{email}</strong>
					</p>
					<p class="text-sm text-cocoa-500">
						{$t('auth.login.linkExpiry')}
					</p>
					<button
						onclick={() => { emailLinkSent = false; email = ''; }}
						class="mt-6 text-primary-600 hover:text-primary-500 font-medium text-sm"
					>
						{$t('auth.login.differentEmail')}
					</button>
				</div>
			{:else if showEmailLogin}
				...
				<form onsubmit={handleEmailLinkSubmit} class="space-y-4">
					<div>
						<label for="email" class="label">{$t('auth.login.emailAddress')}</label>
						<input
							id="email"
							type="email"
							bind:value={email}
					...
					<button type="submit" disabled={emailLinkLoading} class="btn-primary w-full">
						{emailLinkLoading ? $t('auth.login.sending') : $t('auth.login.sendLoginLink')}
					</button>
				</form>
			...
			{:else}
				...
				<div class="relative my-6">
					...
					<div class="relative flex justify-center text-sm">
						<span class="px-2 bg-warm-paper text-cocoa-500">{$t('auth.login.orSignInWithPassword')}</span>
					</div>
				</div>
				...
				<form onsubmit={handleSubmit} class="space-y-4">
					<div>
						<label for="email-login" class="label">{$t('auth.login.email')}</label>
						<input
							id="email-login"
							type="email"
							bind:value={email}
		...
		<div class="mt-6 text-center text-sm text-cocoa-500">
			<p>
				{$t('auth.login.needHelp')} <a href="/faq" class="text-primary-600 hover:text-primary-500 font-medium">{$t('auth.login.faq')}</a>
			</p>
		</div>
	</div>
</div>
